 ...
 * Value: 89,960,763 = (0.003569 * (16 * 4,096) * 10,000 * 10) / 2.6
 */
int g_TemperatureGradient = (int) (ADC_SLOPE * ADC_COUNT_RESOLUTION_MULTIPLIER * ADC_MULTIPLIER * TEMP_MULTIPLIER / ADC_REF_2V6);
void Default_gradient()
{
  g_TemperatureGradient = (int) (ADC_SLOPE * ADC_COUNT_RESOLUTION_MULTIPLIER * ADC_MULTIPLIER * TEMP_MULTIPLIER / ADC_REF_2V6);
}

/*!
 * Global integer value in which the y-intercept of the temperature curve is stored.
 * Value: 392,857 = (1.558576 * (16 * 4,096) * 10) / 2.6
 */
int g_TemperatureIntercept = (int) (ADC_INTERCEPT * ADC_COUNT_RESOLUTION_MULTIPLIER * TEMP_MULTIPLIER / ADC_REF_2V6);
void Default_intercept()
{
  g_TemperatureIntercept = (int) (ADC_INTERCEPT * ADC_COUNT_RESOLUTION_MULTIPLIER * TEMP_MULTIPLIER / ADC_REF_2V6);
}
 ...
 */
void SystemTemperatureByADCValue(uint32_t ulADCValue, int* TempValue, int nDecimal)
{
  int64_t llValue;

  llValue = (g_TemperatureIntercept / TEMP_MULTIPLIER - ((int) ulADCValue));
  llValue *= ADC_MULTIPLIER * nDecimal;
  llValue /= g_TemperatureGradient / TEMP_MULTIPLIER;
  *TempValue = (int) llValue;
}
#else /* SYSTEM_TEMPERATURE_FLOATING_POINT_SUPPORT */
/*!
 * Global integer where the steepness of the temperature curve is stored.
 V = -0.003569 * T + 1.558576 
 T = (1.558576 - V) / 0.003569
 
 slope  : -0.003569 2pow12: 4096
 offset : 1.558576 
 */
float g_TemperatureGradient = -5.622547692307692307;      // Slope: -0.003569 * 4096 / 2.6

/*!
 * Global integer where the y-intercept of the temperature curve is stored.
 */
float g_TemperatureIntercept = 2455.356652307692307692;  // Intercept: 1.558576 * 4096 / 2.6
